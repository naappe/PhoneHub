...
class MediaSessionManager:
    def __init__(self):
        self.process:subprocess.Popen|None=None; self.kind=""; self.scrcpy=shutil.which("scrcpy")
    @property
    ...
    def mark_transport_lost(self):
        """Forget dead media without erasing the user's requested session."""
        if self.process is not None and self.process.poll() is not None:self.process=None
    def _start(self,args:list[str],kind:str):
        if not self.scrcpy:return False,"scrcpy.exe not found in Windows PATH"
        self.stop(); flags=subprocess.CREATE_NO_WINDOW if hasattr(subprocess,"CREATE_NO_WINDOW") else 0
        try:
            self.process=subprocess.Popen([self.scrcpy,*args],creationflags=flags); self.kind=kind; return True,f"{kind.title()} opened"
