...
class Card(QFrame):
    def __init__(self,title,value="—",detail=""):
        ...
        lay=QVBoxLayout(self);lay.setContentsMargins(20,18,20,18);lay.setSpacing(5)
        ...
        lay.addWidget(t);lay.addWidget(self.value);lay.addWidget(self.detail)
...
class Window(QMainWindow):
    ...
    def home(self):
        ...
        h=QLabel("Home");h.setObjectName("heading");self.connection=QLabel("Looking for your phone…");self.connection.setObjectName("status")
        l.addWidget(h);l.addWidget(self.connection)
        row1=QHBoxLayout();self.device=Card("DEVICE");self.battery=Card("BATTERY");self.network=Card("NETWORK")
        for x in [self.device,self.battery,self.network]:row1.addWidget(x)
        l.addLayout(row1)
        row2=QHBoxLayout();self.storage=Card("STORAGE");self.memory=Card("MEMORY");self.android=Card("ANDROID")
    ...
    def refresh(self):
        ...
        if not ds:
            self.connection.setText("● Waiting for PhoneHub Companion");return
        d=ds[0]
        try:
            s=self.server.device_status(d)
            if s.get("type")!="device_status":raise RuntimeError(s.get("message","status unavailable"))
            self.connection.setText(f"● Connected securely  •  {d.address}")
            self.device.value.setText(s.get("device_name","Android"));self.device.detail.setText("Encrypted Companion")
            bp=s.get("battery_percent",0);self.battery.value.setText(f"{bp}%");self.battery.detail.setText("Charging" if s.get("charging") else "Not charging")
            self.network.value.setText(s.get("network","—"));self.network.detail.setText("Active connection")
            total=s.get("storage_total",0);free=s.get("storage_free",0);self.storage.value.setText(gb(free));self.storage.detail.setText(f"free of {gb(total)}")
            mt=s.get("memory_total",0);mf=s.get("memory_free",0);self.memory.value.setText(gb(mf));self.memory.detail.setText(f"available of {gb(mt)}")
    ...
    def apply_style(self):
        ...
        #heading{font-size:28px;font-weight:700;color:#111827} #status{color:#16803a;font-weight:600}
        #card{background:#ffffff;border:1px solid #e3e8f0;border-radius:14px;min-height:130px}
        #cardTitle{color:#7a8599;font-size:11px;font-weight:700} #cardValue{font-size:23px;font-weight:700;color:#182033} #cardDetail{color:#6b768a}
        """)
