...
class CompanionService : Service() {
    ...
    private fun process(req:JSONObject,key:ByteArray):JSONObject{
        ...
        return when(req.optString("type")){
            ...
            "screen_status"->response.put("type","screen_status").put("active",ScreenCaptureService.active).put("width",ScreenCaptureService.frameWidth).put("height",ScreenCaptureService.frameHeight).put("last_frame_at",ScreenCaptureService.lastFrameAt)
            "screen_snapshot"->{val frame=ScreenCaptureService.latestJpeg;if(!ScreenCaptureService.active||frame==null)response.put("type","screen_unavailable").put("message","Open PhoneHub Companion on the phone and tap Start screen sharing.") else response.put("type","screen_snapshot").put("format","jpeg").put("width",ScreenCaptureService.frameWidth).put("height",ScreenCaptureService.frameHeight).put("captured_at",ScreenCaptureService.lastFrameAt).put("image",android.util.Base64.encodeToString(frame,android.util.Base64.NO_WRAP))}
            ...
            "capabilities"->response.put("type","capabilities").put("notifications",false).put("files",true).put("camera",false).put("screen_control",false).put("app_inventory",true).put("remote_app_policy",true).put("screen_capture",true)
